		  ...
		  string =
		    ...
		    results : mapper : error :
		      if builtins.typeOf results != "list" then { success = false ; value = [ error ] ; }
		        ...
		        let
			  success = builtins.trace ( builtins.concatStringsSep "," ( builtins.map ( test : builtins.elemAt test 0 ) ( builtins.filter ( test : ! test.success ) ( builtins.map mapper results ) ) ) ) ( builtins.all ( test : test.success ) ( builtins.map mapper results ) ) ;
